...
├── test_monochrome.py         # Test DICOM reading & MONO1/MONO2 detection
...
|---|---|---|
| `--input-dir` | — | Thư mục chứa DICOM/PNG (tìm đệ quy, hỗ trợ .dcm và .dicom) |
...
|---|---|---|
| **MONOCHROME1** | Pixel thấp = mô đặc (sáng), pixel cao = nền (tối) | **Tự động invert** (`max - pixel`) + adjust Window Center |
| **MONOCHROME2** | Pixel cao = mô đặc (sáng) — chuẩn hiển thị | Giữ nguyên, không invert |

### MONOCHROME1 Window Center Adjustment

Khi xử lý ảnh MONOCHROME1, ngoài việc invert pixel, chương trình còn **tự động điều chỉnh Window Center**:

```
# Pixel gốc MONO1:  range=[65, 10000], WC=3630, WW=7260
# Sau invert:       pixel_mới = max - pixel_cũ
# Window Center:    WC_mới = max - WC_cũ = 10000 - 3630 = 6370
```

**Lý do:** Window Center/Width trong DICOM header được calibrate cho pixel GỐC.
Sau khi invert pixel mà không adjust WC, windowing sẽ clip sai vùng → ảnh bị sai contrast.

### Test MONOCHROME detection (không cần GPU)

```bash
# Chỉ cần pydicom + numpy, không cần YOLOX/TensorRT
python test_monochrome.py --input-dir /path/to/dicoms
```
...
Output: /path/to/output
  ...
  ----------------------------------------
  ! 5 image(s) were MONOCHROME1 -> pixels inverted + WC adjusted
    - IM-0001-0001.dcm
    - IM-0001-0003.dcm
    - ...
============================================================
